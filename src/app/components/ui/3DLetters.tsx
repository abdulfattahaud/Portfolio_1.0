...
export default function TDLetters() {
  ...
  const [scale, setScale] = useState(viewport.width / 4.35);
  const [yPos, setYPos] = useState(viewport.height / 2);

  useEffect(() => {
    setScale(viewport.width / 4.35);
    setYPos(viewport.aspect * 2);
  }, [viewport.width, viewport.aspect]);
  return (
    <motion.group scale={scale} position={[0, yPos, -10]}>
